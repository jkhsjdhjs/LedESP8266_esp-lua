...
local function PCA9685:readRegister(reg, wsh)
...
local function PCA9685:writeRegister(reg, val, wsh)
...
local function PCA9685.initialize(i2c, address, channel, tmr_ref, wsh)
    self.i2c = i2c
...
-- returns true on success, false otherwise
local function PCA9685:setChannelBrightness(channel, brightness, wsh)
...
local function PCA9685:getChannelBrightness(channel, wsh)
    local register = self.channelToRegister(channel)
...
-- returns true on success, false otherwise
local function PCA9685:setColor(red, green, blue, wsh)
...
-- returns true on success, false otherwise
local function PCA9685:fadeToColor(red, green, blue, time, wsh)
    if not time or type(time) ~= "number" and time == 0 then
        return self:setColor(red, green, blue, wsh)
    end
    if wsh then
        wsh:sendAll("info", "fading_start", {
            color = {
                red = red,
                green = green,
                blue = blue
